...
import EditInfoModal from "./EditInfo";
import EditPass from "./EditPass";

function ManagementModal({userInfo, close}) {
    ...
    return (
        <>
            <ul className="edit-form-menu" onClick={changeMenu}>
                <li data-menu='info' className={selectedMenu == 'info' && 'selected'}>기본정보</li>
                <li data-menu='pass' className={selectedMenu == 'pass' && 'selected'}>비밀번호 변경</li>
            </ul>
            {
                selectedMenu == 'info' &&
                <EditInfoModal userInfo={userInfo} close={close}/>
            }
                       {
                selectedMenu == 'pass' &&
                <EditPass userInfo={userInfo} close={close}/>
            }
        </>
    );
}
